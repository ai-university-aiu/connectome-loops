...
# The loop's behaviour is proven end-to-end by bin/run_slice.sh (the closure
# verdict is its real test); these suites cover the dynamics and structure packs
...
# The loop and neural_lattice need PrologAI's lattice and actors packs.
LIB="$LIB -p library=$PROLOGAI_HOME/packs/lattice/prolog"
LIB="$LIB -p library=$PROLOGAI_HOME/packs/actors/prolog"
# causal_map needs the Causalontology engine and conformance harness.
